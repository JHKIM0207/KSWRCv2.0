...
def recent_wind(now: datetime) -> dict:
    ...
    series: dict[str, list[tuple[int, float]]] = {key: [] for key in fields}
    ...
    missing = [key for key in fields if not series[key]]
    if missing:
        raise RuntimeError(f"No solar-wind samples are available for: {', '.join(missing)}")

    for values in series.values():
        values.sort(key=lambda item: item[0])
    latest_by_field = {key: values[-1][0] for key, values in series.items()}
    stale = [
        key for key, timestamp in latest_by_field.items()
        if now - datetime.fromtimestamp(timestamp / 1000, timezone.utc) > MAX_INPUT_AGE
    ]
    if stale:
        raise RuntimeError(f"Solar-wind inputs are stale for: {', '.join(stale)}")

    # The wind and magnetic feeds do not necessarily use identical timestamps.
    # Use the latest time shared by all four series and independent 15-minute medians.
    analysis_ms = min(latest_by_field.values())
    window_start = analysis_ms - 15 * 60 * 1000
    windows = {
        key: [value for timestamp, value in values if window_start <= timestamp <= analysis_ms]
        for key, values in series.items()
    }
    empty = [key for key, values in windows.items() if not values]
    if empty:
        raise RuntimeError(f"No synchronized 15-minute samples are available for: {', '.join(empty)}")

    result = {key: float(np.median(values)) for key, values in windows.items()}
    result["time"] = datetime.fromtimestamp(analysis_ms / 1000, timezone.utc)
    result["sample_count"] = min(len(values) for values in windows.values())
    print(
        "Solar-wind samples: "
        + ", ".join(f"{key}={len(windows[key])}" for key in fields)
    )
    return result
...
def fetch_dst(now: datetime) -> tuple[float, datetime]:
    ...
    for url in NOAA_DST_URLS:
        request = urllib.request.Request(url, headers={"User-Agent": "KSWRC-GitHub-Actions/1.0"})
        try:
    ...
    if rows is None:
        raise RuntimeError("All NOAA Dst sources failed: " + "; ".join(errors))

    candidates: list[tuple[datetime, float]] = []
    header = rows[0] if rows and isinstance(rows[0], list) and all(isinstance(item, str) for item in rows[0]) else None
    data_rows = rows[1:] if header else rows
    for row in data_rows:
        if header and isinstance(row, list):
            row = dict(zip(header, row))
        if isinstance(row, dict):
            time_value = row.get("time_tag") or row.get("time") or row.get("timestamp")
            dst_value = row.get("dst")
        elif isinstance(row, list) and len(row) >= 2:
            time_value, dst_value = row[0], row[1]
        else:
            continue
        try:
